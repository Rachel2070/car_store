    ...
    with pyodbc.connect(connection_str) as connection:
        ...
        if new_car:
            ...
            values = (new_car['model'], new_car['color'], new_car['manufacturer'], new_car['manufacturer_date'],
                      new_car['license_num'], new_car['price'], new_car['num_seats'], new_car['car_pic'])
    ...
    with pyodbc.connect(connection_str) as connection:
        ...
        if updated_car:
            ...
            update_query = """UPDATE cars SET model = ?, color = ?, manufacturer = ?, manufacturer_date = ?, license_num = ?, price = ?, num_seats = ?, car_pic = ?
                              WHERE car_id = ?"""
            values = (updated_car['model'], updated_car['color'], updated_car['manufacturer'], updated_car['manufacturer_date'], updated_car['license_num'], updated_car['price'], updated_car['num_seats'], updated_car['car_id'], updated_car['car_pic'])
...
new_car = {
    ...
    'license_num': 'A852C5423',
    'price': 30000,
    'num_seats': 5,
}
result = get_car_by_id(2017)
print(result)
